...
async function apiCall(api,body){
    const uri=`https://library.jmonkeyengine.org/${api}`;
    console.info("Fetch ",uri,"with payload",body);
...
async function main() {
    try {
        ...
        for (const mediaData of media) {
            ...
            const resp = await apiCall("media/set", mediaData);
        ...
        // publish entry
        {
            ...
            let resp = await apiCall("entry/set", data);
        }
